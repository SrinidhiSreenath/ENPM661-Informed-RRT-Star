...
[RRT, goalNode, Paths, updatepath] = informedrrtstarpathplan(xys, xyg, x, y,A1,A2,A3,B1,B2,B3,C1,C2,C3);
toc
%% Finding shortest path after A*
% test = goalNode;
% path = RRT.nodes(test,:);
% 
% index = 0;
...
%     if j~=1
%     pa = path(j-1,:);
%     line([xy(1) pa(1)], [xy(2) pa(2)], 'Linewidth',2,'Color','r')
%     end
%     
%     pause(0.01) 
%     
...
v = VideoWriter('InformedRRT.avi');
open(v);

iter = 1;

for i=1:1:size(RRT.nodes,1)
    if i == updatepath(iter)               
        for j=size(Paths,1):-1:1
            xy=Paths(j,:, iter);
            ...
            pa = Paths(j-1,:,iter);
                if mod(iter,2) == 0
                    line([xy(1) pa(1)], [xy(2) pa(2)], 'Linewidth',4,'Color','r')
                else
                    line([xy(1) pa(1)], [xy(2) pa(2)], 'Linewidth',4,'Color','g')
                end
            end
